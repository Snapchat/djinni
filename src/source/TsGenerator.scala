...
class TsGenerator(spec: Spec, valdiMode: Boolean) extends Generator(spec) {

  private val tsOutFolder = if (valdiMode) spec.valdiTsOutFolder else spec.tsOutFolder
  ...
  private def tsPrimitiveType(p: MPrimitive): String = p._idlName match {
    case "bool" => "boolean"
    case "i64" => if (valdiMode) "Long" else "bigint"
    case _ => "number"
  }
  ...
  def references(m: Meta): Seq[TsSymbolRefBase] = m match {
    ...
    case MProtobuf(name, _, ProtobufMessage(_,_,_,Some(ts),_)) =>
      if (valdiMode) List(ProtoSymbolRef(name, ts.module, ts.ns))
  ...
  private def fixDjinniSupportModulePath(module: String) =
    if (valdiMode) module.replaceAll("^@djinni_support/", "djinni_support/src/")
    else module
  ...
  override def generate(idl: Seq[TypeDecl]) {
    createFile(tsOutFolder.get, spec.tsModule + ".ts", (w: IndentWriter) => {
