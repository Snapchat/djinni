// AUTOGENERATED FILE - DO NOT MODIFY!
// This file was generated by Djinni from map.djinni

#include "map_list_record.h"
#include "djinni_c_helpers.hpp"
#include "map_list_record.hpp"

testsuite_map_list_record_ref testsuite_map_list_record_new(djinni_array_ref map_list) 
{
    return ::djinni::c_api::Record<::testsuite::MapListRecord>::make(::djinni::c_api::List<std::unordered_map<std::string, int64_t>>::toCpp(map_list, [](auto&& value) { return ::djinni::c_api::Map<std::string, int64_t>::toCpp(std::forward<decltype(value)>(value), [](auto key, auto value) { return std::make_pair(::djinni::c_api::String::toCpp(key), ::djinni::c_api::Number::toCpp<int64_t>(value)); }); }));
}

djinni_array_ref testsuite_map_list_record_get_map_list(testsuite_map_list_record_ref instance)
{
    return ::djinni::c_api::List<std::unordered_map<std::string, int64_t>>::fromCpp(::djinni::c_api::Record<::testsuite::MapListRecord>::toCpp(instance).map_list, [](auto&& value) { return ::djinni::c_api::Map<std::string, int64_t>::fromCpp(std::forward<decltype(value)>(value), [](auto key, auto value) { return std::make_pair(::djinni::c_api::String::fromCpp(key), ::djinni::c_api::Number::fromCpp<int64_t>(value)); }); });
}

void testsuite_map_list_record_set_map_list(testsuite_map_list_record_ref instance, djinni_array_ref value)
{
    ::djinni::c_api::Record<::testsuite::MapListRecord>::toCpp(instance).map_list = ::djinni::c_api::List<std::unordered_map<std::string, int64_t>>::toCpp(value, [](auto&& value) { return ::djinni::c_api::Map<std::string, int64_t>::toCpp(std::forward<decltype(value)>(value), [](auto key, auto value) { return std::make_pair(::djinni::c_api::String::toCpp(key), ::djinni::c_api::Number::toCpp<int64_t>(value)); }); });
}


