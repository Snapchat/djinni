...
final class ProtoTest: XCTestCase {
    func testSwiftToNative() throws {
        ...
        let r = try ProtoTests_statics.protoToStrings(x:proto)
    ...
    func testEmbeddedProto() throws {
        ...
        let s = try ProtoTests_statics.embeddedProtoToString(x:rec)
        XCTAssertEqual(s, p.name)
        let rec2 = try ProtoTests_statics.stringToEmbeddedProto(x:s);
    ...
    func testBoxing() throws {
        ...
        let p2 = Person.with {$0.name = "jerry"; $0.id = 2}
        var plist = [p1, p2]
        let slist = try ProtoTests_statics.protoListToStrings(x:plist)
        XCTAssertEqual(slist[0], "tom")
        XCTAssertEqual(slist[1], "jerry")

        plist = try ProtoTests_statics.stringsToProtoList(x:slist)
        XCTAssertEqual(plist[0].name, "tom")
        XCTAssertEqual(plist[1].name, "jerry")

        let s = try ProtoTests_statics.optionalProtoToString(x:p1)
        XCTAssertEqual(s, "tom")
        let p = try ProtoTests_statics.stringToOptionalProto(x:"tom")
        XCTAssertEqual(p?.name, "tom")

        let r = try ProtoTests_statics.stringToProtoOutcome(x:"tom")
