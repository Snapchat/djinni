...
struct SupplierFunctionHolder {
    virtual ~SupplierFunctionHolder() = default;
    virtual jobject call(JNIEnv* env) = 0;
};

// Template implementation for specific types
template<typename T>
struct TypedSupplierFunctionHolder : SupplierFunctionHolder {
    using CppType = typename T::CppType;
    using CppSupplierType = std::function<CppType()>;
    
    CppSupplierType func;
    
    TypedSupplierFunctionHolder(CppSupplierType f) : func(std::move(f)) {}
...
struct NativeSupplierHandlerJniInfo {
    const GlobalRef<jclass> clazz { jniFindClass("com/snapchat/djinni/NativeSupplierHandler") };
...
public:
    ...
    static LocalRef<JniType> fromCpp(JNIEnv* jniEnv, const CppType& c) {
        ...
        // The implementation is in Supplier_jni.cpp
        auto* holder = new TypedSupplierFunctionHolder<T>(c);
        
        // Create a Java NativeSupplierHandler that calls back to C++
        const auto& nativeSupplierHandlerJniInfo = JniClass<NativeSupplierHandlerJniInfo>::get();
        
        auto handler = LocalRef<jobject>(jniEnv, jniEnv->NewObject(
            nativeSupplierHandlerJniInfo.clazz.get(),
            nativeSupplierHandlerJniInfo.constructor,
