 ...
 * by the JNI layer through LazyAdaptor::fromCpp.
 */
final class NativeLazyHandler<T> implements Supplier<T> {
    private long mNativeRef;

    NativeLazyHandler(long nativeRef) {
    ...
    public T get() {
        if (mNativeRef == 0) {
            throw new IllegalStateException("NativeLazyHandler has been destroyed");
        }
    ...
    // These native methods are implemented in the generated JNI code
    // Each instantiation of LazyAdaptor<T> will generate the appropriate
