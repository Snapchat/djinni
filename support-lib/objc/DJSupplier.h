...
/**
 * DJSupplier represents a supplier of a value - computed when get is called.
 *
 ...
 *
 * Example:
 *   DJSupplier<NSString *> *supplierValue = [DJSupplier supplierWithBlock:^NSString *() {
 *       return @"computed value";
 *   }];
 *   NSString *result = [supplierValue get];  // Compute and get the value
 */
@interface DJSupplier<__covariant Value> : NSObject

- (nonnull instancetype)init NS_UNAVAILABLE;

/**
 * Creates a supplier from a block.
 * The block will be called when `get` is invoked.
 */
+ (nonnull instancetype)supplierWithBlock:(Value _Nonnull (^_Nonnull)(void))block;

/**
 * Initializes a supplier with a block.
 * The block will be called when `get` is invoked.
 */
- (nonnull instancetype)initWithBlock:(Value _Nonnull (^_Nonnull)(void))block NS_DESIGNATED_INITIALIZER;

/**
 * Gets the value by executing the supplier.
